...
L<XML::Compile::SOAP|XML::Compile::SOAP>.

The CODE reference provided will be called with the request
message (HTTP::Request) as first parameter, and the user
agent (LWP::UserAgent) as second.  You must return an answer
(HTTP::Response) or C<undef>.
...
XML::Compile::SOAP::HTTPClient-E<gt>B<fakeServer>([FAKE|undef])
